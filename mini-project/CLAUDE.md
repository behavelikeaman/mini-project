...
- 자동화된 테스트 스위트는 없습니다. 변경 사항은 브라우저에서 직접 확인하세요: 실제 카카오톡 내보내기 파일을 업로드해 파싱된 메시지 수/날짜 범위를 확인하고, 네트워크 탭/콘솔에 에러가 없는지 확인합니다. 파서 회귀만 빠르게 격리해서 확인하고 싶다면, `js/parser.js`의 IIFE 본문을 별도 `.js` 파일로 복사해 Node로 실행하고 `fs.readFileSync`로 샘플 내보내기 파일을 읽여 테스트하는 방법이 브라우저 없이도 유용합니다.
