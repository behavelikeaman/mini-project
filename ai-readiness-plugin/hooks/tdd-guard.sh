...
# 원본: https://github.com/jha0313/demo-project/blob/main/.claude/hooks/tdd-guard.sh
# 원본과 다른 점 두 가지:
#   1. jq 대신 node로 JSON을 파싱한다 (jq가 없는 환경 대응, 로직은 동일).
#   2. Windows 역슬래시 경로를 정규화한다 (아래 주석 참고).

INPUT=$(cat)
FILE_PATH=$(printf '%s' "$INPUT" | node -e 'try{const o=JSON.parse(require("fs").readFileSync(0,"utf8"));process.stdout.write(o.tool_input&&o.tool_input.file_path?o.tool_input.file_path:"")}catch(e){}')
...
# Windows에서는 도구가 역슬래시 경로(C:\...\src\components\X.tsx)를 넘긴다.
# 아래 면제 패턴(*/components/*, */types/*, */page.tsx 등)은 슬래시 기준이라
# 정규화하지 않으면 면제가 전부 무력해진다.
FILE_PATH=${FILE_PATH//\\//}
...
case "$FILE_PATH" in
  *.ts|*.tsx|*.js|*.jsx)
    ...
    if [ "$TEST_FOUND" = false ]; then
      # 프로젝트가 더 큰 저장소의 하위 폴더일 수 있으므로 git 루트보다 프로젝트 폴더를 우선한다
